...
std::pair<Buffer*, u32> BufferCache::ObtainBuffer(VAddr device_addr, u32 size, bool is_written,
                                                  ...
                                                  bool skip_stream_buffer) {
    // For read-only buffers use device local stream buffer to reduce renderpass breaks.
    // Skip the fast path in two cases:
    //   1. The caller (BindBuffers, when the dispatch contains any write descriptor) asks us
    //      to. Within a single compute dispatch that mixes reads with writes, splitting reads
    //      onto stream_buffer while writes go to a dedicated buffer leaves the descriptor set
    //      with two physically distinct VkBuffers and NVIDIA's driver hangs on that pattern
    //      (seen in GT7 cs_0x1c0f802e).
    //   2. A dedicated buffer is already registered for this range — otherwise a read snapshot
    //      in stream_buffer aliases a dedicated buffer for the same guest VA, same problem.
    if (!skip_stream_buffer && !is_written && size <= CACHING_PAGESIZE &&
        !IsRegionGpuModified(device_addr, size) && IsRegionCpuModified(device_addr, size) &&
        !IsRegionRegistered(device_addr, size)) {
